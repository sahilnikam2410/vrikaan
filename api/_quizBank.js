/**
 * VRIKAAN Academy — mock-test question bank. SERVER ONLY.
 *
 * This file holds the correct answers. It used to live in src/lib/mockTests.js,
 * which ships to the browser — so every answer was in the bundle and a score
 * was whatever the client chose to report. Certificates carried that number.
 * Questions are now served without `ans`/`why`, grading happens here, and the
 * certificate endpoint reads the score off the graded attempt.
 *
 * Underscore-prefixed so Vercel ignores it as a function.
 *
 * Each question: { q, opts[], ans (index), why }.
 */
...
// Mirrors TEST_META in src/lib/mockTests.js; `pick` stays server-side because
// it determines which questions get served.
export const TESTS = [
  { id: "fundamentals", title: "Cyber Safety Fundamentals", timeSec: 300, pass: 70, pick: { upi: 2, phishing: 2, vishing: 2, passwords: 2, privacy: 1, breach: 1 } },
  { id: "upi-defender", title: "UPI Fraud Defender", timeSec: 240, pass: 80, pick: { upi: 5, vishing: 2, breach: 1 } },
  { id: "phishing-pro", title: "Phishing & Scam-Call Pro", timeSec: 240, pass: 80, pick: { phishing: 5, vishing: 3 } },
  { id: "account-armor", title: "Account Armor", timeSec: 240, pass: 80, pick: { passwords: 5, privacy: 3, breach: 2 } },
  { id: "loan-job-shield", title: "Loan & Job Scam Shield", timeSec: 240, pass: 80, pick: { loanApp: 5, jobScam: 5 } },
  { id: "shopping-safety", title: "Online Shopping Safety", timeSec: 240, pass: 80, pick: { shopping: 5, phishing: 2, upi: 3 } },
  { id: "deepfake-defense", title: "AI & Deepfake Defense", timeSec: 240, pass: 80, pick: { deepfake: 5, vishing: 3, social: 2 } },
  { id: "social-shield", title: "Social Media Shield", timeSec: 240, pass: 80, pick: { social: 5, privacy: 3, breach: 2 } },
  { id: "master-exam", title: "VRIKAAN Master Exam", timeSec: 600, pass: 85, pick: { upi: 3, phishing: 2, vishing: 2, passwords: 2, loanApp: 2, jobScam: 2, social: 2, shopping: 2, deepfake: 2, breach: 1 } },
];
...
/**
 * Pick a shuffled question set for a test.
 * Returns refs — [{ cat, i }] — rather than the questions themselves, so an
 * attempt can be stored without duplicating the bank into Firestore.
 */
export function pickQuestionRefs(test) {
  const refs = [];
  for (const [cat, n] of Object.entries(test.pick)) {
    const pool = (QUESTIONS[cat] || []).map((_, i) => ({ cat, i }));
    shuffle(pool);
    refs.push(...pool.slice(0, n));
  }
  shuffle(refs);
  return refs;
}

/** Resolve refs to questions WITHOUT the answer — safe to send to a browser. */
export function publicQuestions(refs) {
  return refs.map((r) => {
    const q = QUESTIONS[r.cat]?.[r.i];
    return q ? { q: q.q, opts: q.opts } : null;
  }).filter(Boolean);
}

/** Resolve refs to full questions, including `ans` and `why`. Server only. */
export function fullQuestions(refs) {
  return refs.map((r) => QUESTIONS[r.cat]?.[r.i]).filter(Boolean);
}
