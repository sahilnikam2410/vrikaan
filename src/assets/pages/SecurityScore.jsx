        ...
        {score === null ? (
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 18, padding: "36px 32px", backdropFilter: "blur(8px)" }}>
            ...
            {questions.map((q, i) => (
              <div key={q.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 0", borderBottom: i < questions.length - 1 ? `1px solid ${T.border}` : "none" }}>
                <div style={{ display: "flex", gap: 14, alignItems: "center", flex: 1 }}>
                  <span style={{ fontFamily: "'Vrikaan Mono', monospace", fontSize: 11, color: T.mutedDark, minWidth: 24 }}>{String(i + 1).padStart(2, "0")}</span>
            ...
            <div style={{ textAlign: "center", marginTop: 32 }}>
              <button onClick={calcScore} disabled={answered < questions.length}
                style={{ padding: "14px 40px", background: answered === questions.length ? T.accent : T.mutedDark, color: "#fff", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: answered === questions.length ? "pointer" : "default", fontFamily: "'Vrikaan Sans', sans-serif", opacity: answered === questions.length ? 1 : 0.5, transition: "all 0.3s" }}>
                Calculate My Score
              </button>
            </div>
          </div>
          ...
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 18, padding: "48px 32px", backdropFilter: "blur(8px)", textAlign: "center" }}>
            <svg width="180" height="180" viewBox="0 0 180 180">
              ...
              <text x="90" y="82" textAnchor="middle" fontFamily="'Vrikaan Sans', sans-serif" fontSize="48" fontWeight="800" fill={scoreColor}>{score}</text>
              <text x="90" y="108" textAnchor="middle" fontFamily="'Vrikaan Mono', monospace" fontSize="10" fill={T.muted} letterSpacing="2">OUT OF 100</text>
            </svg>
            <div style={{ fontFamily: "'Vrikaan Sans', sans-serif", fontSize: 28, fontWeight: 700, color: scoreColor, marginTop: 20 }}>{scoreLabel}</div>
            ...
            <div style={{ marginTop: 32, textAlign: "left", background: "rgba(0,0,0,0.2)", borderRadius: 12, padding: 24 }}>
              <h3 style={{ fontFamily: "'Vrikaan Sans', sans-serif", fontSize: 16, fontWeight: 600, margin: "0 0 16px" }}>Your Results</h3>
              {questions.map((q, i) => (
            ...
            <div style={{ marginTop: 28, display: "flex", gap: 12, justifyContent: "center" }}>
              <button onClick={() => { setScore(null); setAnswers({}); }} style={{ padding: "12px 28px", background: "transparent", border: `1px solid ${T.border}`, borderRadius: 10, color: T.muted, cursor: "pointer", fontFamily: "'Vrikaan Sans', sans-serif", fontSize: 13, fontWeight: 600 }}>Retake Assessment</button>
