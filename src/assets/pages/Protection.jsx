          ...
          {layers.map((l, i) => (
            <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: "32px 28px", position: "relative", overflow: "hidden", transition: "all 0.3s" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: 16 }}>
                ...
                <span style={{ padding: "4px 12px", borderRadius: 100, background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.15)", fontSize: 10, fontWeight: 600, color: "#22c55e", fontFamily: "'Vrikaan Mono', monospace" }}>{l.status}</span>
              </div>
              <h3 style={{ fontFamily: "'Vrikaan Sans', sans-serif", fontSize: 19, fontWeight: 600, margin: "0 0 10px" }}>{l.title}</h3>
        ...
        <div className="resp-grid-4" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 64 }}>
          {[{ val: "AI", label: "Powered Detection", color: T.cyan }, { val: "<2s", label: "Scan Time", color: T.accent }, { val: "24/7", label: "Always-on", color: "#22c55e" }, { val: "EN+हिं", label: "Bilingual", color: T.ember }].map((s, i) => (
            <div key={i} style={{ textAlign: "center", padding: "28px 16px", background: T.card, border: `1px solid ${T.border}`, borderRadius: 14 }}>
              <div style={{ fontFamily: "'Vrikaan Sans', sans-serif", fontSize: 32, fontWeight: 800, color: s.color }}>{s.val}</div>
